...
| E2 | 主菜单 | `TitleView` 只有「开始」 | 继续 / 新游戏 / **选择存档（槽位列表，显示时间与章节）** / 设置 / 退出；接管标题路由（替换 `MonsterTitleRouter`）；设置面板已可直接调 `SettingsController.OpenAsync()` | Core/UI Views 或新模块 | E1、E3 | M | opus | 待做（下一波） |
| E3 | 设置面板 | `SettingsSaveData` 有音量 / 语言字段，无面板 | 音量三路、语言占位、按键提示；**PC 显示设置**：分辨率列表（取自显示器）、全屏 / 无边框 / 窗口化、垂直同步、帧率上限（见 E8）；写回并落盘 | Core/UI + Core/Save | E5（入口） | M | opus | 完成（2026-09-26）待视觉验收；设置改为跨槽位独立档案 `settings`（分区版本 2）；标题界面尚无「设置」入口 |
